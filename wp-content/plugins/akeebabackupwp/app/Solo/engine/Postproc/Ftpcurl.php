...
class Ftpcurl extends Base
{
	...
	public function processPart($absolute_filename, $upload_as = null)
	{
		...
		$host       = $config->get('engine.postproc.ftpcurl.host', '');
		$port       = $config->get('engine.postproc.ftpcurl.port', 21);
		$user       = $config->get('engine.postproc.ftpcurl.user', '');
		$pass       = $config->get('engine.postproc.ftpcurl.pass', 0);
		$workaround = $config->get('engine.postproc.ftpcurl.passive_mode_workaround', null);
		$directory  = $config->get('volatile.postproc.directory', null);

		if (empty($directory))
		{
			$directory = $config->get('engine.postproc.ftpcurl.initial_directory', '');
		}

		$subdir  = trim($config->get('engine.postproc.ftpcurl.subdirectory', ''), '/');
		$ssl     = $config->get('engine.postproc.ftpcurl.ftps', 0) == 0 ? false : true;
		$passive = $config->get('engine.postproc.ftpcurl.passive_mode', 0) == 0 ? false : true;
		...
		try
		{
			$ftp = new TransferFtpCurl($options);
		}
