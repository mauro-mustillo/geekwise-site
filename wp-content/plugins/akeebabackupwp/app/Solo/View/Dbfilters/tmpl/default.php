...
Solo.loadScripts[Solo.loadScripts.length] = function () {
	(function($){
		...
		// Push translations
		Solo.Dbfilters.translations['UI-ROOT'] = '<?php echo Escape::escapeJS(Text::_('COM_AKEEBA_FILEFILTERS_LABEL_UIROOT')) ?>';
		Solo.Dbfilters.translations['UI-ERROR-FILTER'] = '<?php echo Escape::escapeJS(Text::_('COM_AKEEBA_FILEFILTERS_LABEL_UIERRORFILTER')) ?>';
		<?php
			$filters = array('tables', 'tabledata');
			foreach($filters as $type)
			{
				echo "\tSolo.Dbfilters.translations['UI-FILTERTYPE-" . strtoupper($type)."'] = '".
					Escape::escapeJS(Text::_('COM_AKEEBA_DBFILTER_TYPE_' . $type)) . "';\n";
		...
		<?php
			...
			foreach($types as $type)
			{
				echo "\tSolo.Dbfilters.translations['UI-TABLETYPE-" . strtoupper($type)."'] = '".
					Escape::escapeJS(Text::_('COM_AKEEBA_DBFILTER_TABLE_' . $type)) . "';\n";
			}
		?>
