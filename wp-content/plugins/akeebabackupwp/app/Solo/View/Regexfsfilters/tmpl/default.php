...
Solo.loadScripts[Solo.loadScripts.length] = function () {
	(function($){
		...
		// Push translations
		Solo.Regexfsfilters.translations['UI-ROOT'] = '<?php echo Escape::escapeJS(Text::_('COM_AKEEBA_FILEFILTERS_LABEL_UIROOT')) ?>';
		Solo.Regexfsfilters.translations['UI-ERROR-FILTER'] = '<?php echo Escape::escapeJS(Text::_('COM_AKEEBA_FILEFILTERS_LABEL_UIERRORFILTER')) ?>';
		...
		<?php
			...
			foreach($filters as $type)
			{
				echo "\tSolo.Regexfsfilters.translations['UI-FILTERTYPE-REGEX" . strtoupper($type)."'] = '".
					Escape::escapeJS(Text::_('COM_AKEEBA_FILEFILTERS_TYPE_' . $type)) . "';\n";
			}
		?>
