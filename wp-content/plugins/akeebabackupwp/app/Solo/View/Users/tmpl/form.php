...
Solo.loadScripts[Solo.loadScripts.length] = function () {
	(function($){
		$(document.getElementById('tfa[method]')).change(onTFAMethodChange);

		onTFAMethodChange();

		function onTFAMethodChange()
		{
			$('#tfa_containers>div').each(function(index, container){
				$(container).hide();
			})

			var containerName = 'tfa_' + $(document.getElementById('tfa[method]')).val();
			$('#' + containerName).show();
		}
	}(akeeba.jQuery));
};
