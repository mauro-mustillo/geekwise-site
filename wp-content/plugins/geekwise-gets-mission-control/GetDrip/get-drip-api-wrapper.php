...
Class Drip_Api {
    ...
    private $user_agent = "Geekwise Academy (geekwiseacademy.com)";
    ...
    public function create_or_update_subscriber($params) {
        ...
        $req_params = array('subscribers' => array($params['subscribers']));
    ...
    /**
     * Sends a request to batch add  subscribers and returns its record or false
     * 
     * @param array $params
     * @param array/bool $account
     */
    public function batch_create_or_update_subscribers($params) {
        if (empty($params['account_id'])) {
            throw new Exception("Account ID not specified");
        }
        ...
        // The API wants the params to be JSON encoded
        $req_params = array('batches' => array($params['batches']));
